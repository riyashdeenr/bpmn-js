...
function isOutgoingEventBasedGatewayConnection(connection) {
  ...
  if (
    ...
    connection.businessObject.sourceRef &&
    connection.businessObject.sourceRef.$type
  ) {
    return connection.businessObject.sourceRef.$type === 'bpmn:EventBasedGateway';
  }
}
...
function areOutgoingEventBasedGatewayConnections(connections) {
  ...
  for (var i = 0; i < connections.length; i++) {
    if (isOutgoingEventBasedGatewayConnection(connections[i])) {
      result = true;
      break;
    }
  }
